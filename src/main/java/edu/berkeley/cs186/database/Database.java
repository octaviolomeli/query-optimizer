...
public class Database implements AutoCloseable {
    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        @Override
        public BacktrackingIterator<Record> lookupKey(String tableName, String columnName, DataBox key) {
            Table tab = getTable(tableName);
            tableName = tab.getName();
            BPlusTree tree = indexFromMetadata(getColumnIndexMetadata(tableName, columnName).getSecond());
        ...
        public void close() {
            try {
                // TODO(proj4_part2)
                /* begin solution proj4_part2 */
                /* This is but one way to do it (reverse order of acquisition ensures multigranularity constraints are
                   always satisfied). Alternative ways to do it include performing a topological sort (any topological sort
                   order suffices), looping through all the locks repeatedly with a try/catch until all locks are released,
                   releasing in a literal "bottom-up" order (all page locks, then all table locks, then db lock), and
                   sorting with a comparison-based sort (comparator being: A descendant of B => A < B, A ancestor of B
                   => B > A, otherwise, arbitrary order. Up to the students as to how they want to implement it. See
                   guide for more alternative solutions. */
                List<Lock> locks = lockManager.getLocks(this);
                ListIterator<Lock> lockIter = locks.listIterator(locks.size());
                while (lockIter.hasPrevious()) {
                    ResourceName name = lockIter.previous().name;
                    LockContext context = LockContext.fromResourceName(lockManager, name);
                    context.release(this);
                }
                /* end solution proj4_part2 */
                /* placeholder proj4_part2 return; */
            } catch (Exception e) {
    ...
    private class TransactionImpl extends Transaction {
        ...
        public void createIndex(String tableName, String columnName, boolean bulkLoad) {
            ...
            if (bulkLoad) {
                /* begin solution proj99 */
                // don't mind this
                /*
                SortOperator sortOperator = new SortOperator(
                    transactionContext, tableName,
                    Comparator.comparing((Record a) -> a.getValue(columnIndex))
                );
                tree.bulkLoad(
                    StreamSupport.stream(((Iterable<Record>) sortOperator::iterator).spliterator(), false)
                                 .map((Record r) -> new Pair<>(r.getValue(columnIndex), r))
                                 .iterator(),
                        0.75f
                );
    ...
    public boolean loadCSV(String name) throws IOException {
        return this.loadDelimitedFile("", name, ",", ".csv");
    }

    public boolean loadDelimitedFile(String prefix, String name, String delimiter, String extension) throws IOException {
            String fileName = name + extension;
            if (prefix.length() != 0) {
                fileName = prefix + "/" + fileName;
            }
            InputStream is = Database.class.getClassLoader().getResourceAsStream(fileName);
            InputStreamReader reader = new InputStreamReader(is, StandardCharsets.UTF_8);
            BufferedReader buffered = new BufferedReader(reader);
            String[] header = buffered.readLine().split(delimiter);
