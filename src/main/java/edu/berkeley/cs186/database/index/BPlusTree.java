...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> pushedUp = root.put(key, rid);
        ...
        keys.add(pushedUp.get().getFirst()); // New root key
        children.add(root.getPage().getPageNum()); // Left child (old root)
        children.add(pushedUp.get().getSecond()); // Right child
    ...
    public void bulkLoad(Iterator<Pair<Pair<DataBox, RecordId>, RecordId>> data, float fillFactor) {
        ...
        if (scanAll().hasNext()) {
            throw new BPlusTreeException("Bulk loading failed! Tree is not empty.");
        }

        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> pushedUp = root.bulkLoad(data, fillFactor);
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private Optional<LeafNode> currentLeaf;
        private Iterator<RecordId> currentRIDs;
        ...
        public BPlusTreeIterator() {
            currentLeaf = Optional.of(root.getLeftmostLeaf());
            currentRIDs = currentLeaf.get().scanAll();
            currentLeaf = currentRIDs.hasNext() ? currentLeaf : Optional.empty();
        }
        ...
        public RecordId next() {
            // TODO(proj2): implement
            // Do we still have a leaf node?
            if (!(hasNext() && currentRIDs.hasNext())) {
                throw new NoSuchElementException();
            }

            // Current leaf still has records
            RecordId result = currentRIDs.next();
            if (!currentRIDs.hasNext()) {
                currentLeaf = currentLeaf.flatMap(LeafNode::getRightSibling);
                currentRIDs = currentLeaf.map(LeafNode::scanAll).orElse(null);
            }
            return result;
        }
    }
