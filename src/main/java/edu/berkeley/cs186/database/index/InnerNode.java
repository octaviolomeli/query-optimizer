...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        int index = numLessThanEqual(key, keys); // Get correct index from helper
        BPlusNode recursiveChild = getChild(index); // Could be leaf or inner node
        return recursiveChild.get(key);
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        BPlusNode recursiveChild = getChild(index);
        Optional<Pair<DataBox, Long>> pushedUp = recursiveChild.put(key, rid);

        // No overflow case
        if (!pushedUp.isPresent()) {
            return Optional.empty();
        }

        // Previous overflowed case
        keys.add(index, pushedUp.get().getFirst());
        children.add(index + 1, pushedUp.get().getSecond());
        ...
        // Current node also overflows
        // Redistribute data
        List<DataBox> rightKeys = new ArrayList<>();
        List<Long> rightChildren = new ArrayList<>();

        for (int i = 0; i < metadata.getOrder() + 1; i++) {
            rightKeys.add(keys.remove(metadata.getOrder()));
            rightChildren.add(children.remove(metadata.getOrder() + 1));
        }
        DataBox split_key = rightKeys.remove(0);
        InnerNode newInner = new InnerNode(metadata, bufferManager, rightKeys, rightChildren, treeContext);
        newInner.sync();
        sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext()) {
            if (keys.size() >= 2 * metadata.getOrder() + 1) {
                ...
                // our inner node is not yet filled up

                BPlusNode rightMostChild = getChild(children.size() - 1);
                Optional<Pair<DataBox, Long>> pushedUp = rightMostChild.bulkLoad(data, fillFactor);
                ...
                keys.add(pushedUp.get().getFirst());
                children.add(pushedUp.get().getSecond());
            }
        }
