...
public class TestEvictionPolicy {
    ...
    public void testMRUPolicy() {
        EvictionPolicy policy = new MRUEvictionPolicy();
        policy.init(frames[0]); policy.hit(frames[0]);
        policy.init(frames[1]); policy.hit(frames[1]);
        policy.init(frames[2]); policy.hit(frames[2]);
        policy.init(frames[3]); policy.hit(frames[3]);

        assertEquals(frames[3], policy.evict(new BufferFrame[] {frames[0], frames[1], frames[2], frames[3]}));
        policy.cleanup(frames[3]);
        ...
        assertEquals(frames[4], policy.evict(new BufferFrame[] {frames[4], frames[1], frames[2], frames[0]}));
        policy.cleanup(frames[4]);

        policy.init(frames[5]); policy.hit(frames[5]);

        assertEquals(frames[5], policy.evict(new BufferFrame[] {frames[0], frames[1], frames[5], frames[2]}));
        policy.cleanup(frames[5]);

        policy.init(frames[6]); policy.hit(frames[1]);

        frames[1].unpin();

        assertEquals(frames[1], policy.evict(new BufferFrame[] {frames[6], frames[1], frames[0], frames[2]}));
        policy.cleanup(frames[1]);
    ...
    public void testFIFOPolicy() {
        ...
        policy.hit(frames[2]);
        frames[1].pin();
        assertEquals(frames[2], policy.evict(new BufferFrame[] {frames[1], frames[2], frames[3], frames[4]}));
        policy.cleanup(frames[2]);
        ...
        boolean exceptionThrown = false;
        try {
            policy.evict(new BufferFrame[] {frames[3], placeholderFrames[4], placeholderFrames[1]});
        } catch (IllegalStateException e) {
            exceptionThrown = true;
        }
        assertTrue(exceptionThrown);

        frames[3].unpin(); policy.hit(frames[3]);
        frames[1].unpin(); policy.hit(frames[1]);
        frames[4].unpin();
        policy.init(frames[5]);

        assertEquals(frames[1], policy.evict(new BufferFrame[] {frames[1], frames[3], frames[4], frames[5]}));
        policy.cleanup(frames[1]);
    }
}
